...
TITLE = (
    "Global indicator availability and exposure inequalities in lung cancer prevention among women: "
...
def exposure_inequality_effects(country: pd.DataFrame, n_boot: int = 5000) -> pd.DataFrame:
    specs = [
        ...
        ("Ambient PM2.5", "PM25_2019", "LIC minus HIC", "µg/m³", 1),
...
def plot_incidence_scatter(ax: plt.Axes, model_stats: dict[str, object]) -> None:
    ...
    ax.set_ylabel("Age-standardised incidence among women, z score")
...
def plot_region_distributions(fig: plt.Figure, country: pd.DataFrame) -> None:
    ...
    specs = [
        ...
        ("female_LC_incidence_2021", "Lung cancer incidence among women", "Age-standardised rate per 100,000"),
    ]
...
ABSTRACT = {
    "Background": (
        "Global datasets quantify exposures and lung cancer burden, but their comparability across prevention and care is unclear. We assessed indicator availability, missingness, and inequalities in prevention-related exposures among women using a WHO Health Inequality Data Repository-centred source framework."
    ),
    "Methods": (
        "We conducted a cross-national ecological study of 197 countries and territories. The primary frame comprised current female smoking, clean-fuel access, ambient PM2.5, and age-standardised incidence of tracheal, bronchus, and lung cancer among women. A structured audit assessed exposure, burden, care pathway, and socioeconomically stratified outcome indicators for coverage, disaggregation, comparability, machine readability, uncertainty, and age standardisation. Income-group inequalities used median differences with bootstrap 95% confidence intervals and Cliff's delta. As a data-coherence check, we modelled incidence against an available 2000-2010 proxy for historical smoking among women, GDP, urbanisation, and WHO region, with HC3-robust, influence, and spatial sensitivity analyses."
    ),
    "Results": (
        "All four indicators were available for 163/197 settings; among 34 excluded settings, 28 were missing only female smoking. Completeness ranged from 72.0% in low-income to 89.1% in high-income settings. Compared with high-income countries, low-income countries had a 96.2-percentage-point higher median clean-fuel deficit (95% CI, 89.4 to 98.6) and 24.1 µg/m³ higher median PM2.5 (95% CI, 14.8 to 28.6). Current smoking prevalence among women was 15.9 percentage points higher in high-income countries (95% CI, 10.6 to 18.5). Three of six exposure components and one of four burden components met the criteria. Care-pathway and socioeconomically stratified outcomes were limited or insufficiently integrated. Historical smoking was associated with incidence (standardised coefficient, 0.323; 95% CI, 0.163 to 0.483); the association remained positive in influence and spatial sensitivity analyses, although residual spatial autocorrelation persisted."
    ),
    "Conclusions": (
        "Global data support comparison of selected exposure inequalities but not the full prevention-to-care pathway. The audit identifies comparable indicators and equity-relevant gaps; the incidence model is a data-coherence check, not causal evidence."
    ),
}
...
BACKGROUND = [
    ...
    (
        "Global burden studies estimate lung cancer burden attributable to active and second-hand smoking, ambient and household particulate pollution, and other risks across countries and over time.[14-16] A separate question concerns the underlying information infrastructure: are the indicators needed to compare prevention, diagnosis, treatment, survival, and socioeconomic inequalities in lung cancer outcomes among women available in compatible, disaggregated, and reusable forms? National exposure and burden estimates alone cannot show who is screened, when disease is diagnosed, whether treatment is received, or whether outcomes differ by wealth, education, or residence."
    ),
    (
        "The WHO Health Inequality Data Repository (HIDR) is designed for disaggregated monitoring rather than as a cancer registry or care pathway database.[17,18] The latest HIDR update, released with Health Equity Assessment Toolkit version 7 in March 2026, contains more than 13 million data points across over 2400 indicators, 22 inequality dimensions, and 62 datasets.[17,19] This breadth permits a direct assessment of which prevention inequalities can be measured and where the pathway remains unobserved."
    ),
    (
        "Relevant evidence is distributed across data resources beyond HIDR. WHO GHO and GHE, GBD, tobacco-surveillance systems, CanScreen5, screening trials, CONCORD, subtype estimates, and radon-risk estimates each cover different parts of the pathway.[20-31] Clinical and molecular data are required to study mechanisms such as never-smoker lung cancer and hormonal susceptibility and were outside this ecological analysis.[32,33] We audited the suitability of selected exposure, burden, care pathway, and socioeconomically stratified lung cancer outcome indicators for comparable country-level analysis, then quantified income-group inequalities in the exposures that met the criteria. An explicitly age-standardised incidence model using an available proxy for historical smoking among women served as an epidemiological data-coherence check, not as an estimate of causal effects or unmeasured environmental exposure."
    ),
]
...
METHOD_SECTIONS = [
    ...
    ("Indicator-availability audit", [
        ...
        (
            "Shen Wang and Jing Zhou independently coded all 18 components across the 10 audit dimensions using the predefined categories. After both rounds were complete, the coder-specific records were archived on 13 July 2026 and compared cell by cell. The coders agreed on all 180 cells (100%; Cohen's κ = 1.00), and no item required adjudication by Sha Xiao. Supplementary Table S3 reports both sets of codes, the final classification, and agreement for every cell."
        ),
    ]),
    ("Primary indicators and derived measures", [
        (
            "The primary analytic frame used four non-redundant indicators: female age-standardised current tobacco smoking prevalence in 2022; total population access to clean fuels and technologies for cooking in 2020; national annual mean ambient PM2.5 concentration in 2019; and age-standardised incidence of tracheal, bronchus, and lung cancer among women in 2021. This four-indicator frame avoided counting mortality, YLL, and DALY as separate contributions to a generic completeness score. Coverage of the latter burden outcomes was reported separately for triangulation."
        ),
        (
            "Historical female smoking was the mean of female age-standardised current tobacco smoking prevalence in 2000, 2005, and 2010. The window was chosen to precede the 2021 incidence outcome and to reflect cohort history better than current smoking, but it captures only part of the latency-relevant exposure period. Clean-fuel deficit was calculated as 100% minus the percentage of the population primarily relying on clean fuels and technologies for cooking. Rural clean-fuel disadvantage was urban minus rural clean-fuel access. Clean-fuel deficit is a country-level proxy for household energy poverty, not a direct measurement of individual household air-pollution exposure. Ambient PM2.5 was a modelled national concentration, not personal exposure."
        ),
        (
            "Structural indicators included WHO region, World Bank income group, gross domestic product (GDP) per capita, urbanisation, the proportion aged 65 years or older, HDI, and Socio-demographic Index (SDI).[20-22,34,35] WHO GHE female mortality, YLL, and DALY rates were retained as supplementary outcomes because the downloaded labels did not explicitly state whether, or how, the rates had been age-standardised. The age-standardised incidence estimate was therefore the only burden indicator used in the primary validation model. Indicator definitions, coverage, and analytical roles are summarised in Table 1."
        ),
    ]),
...
RESULT_SECTIONS = [
    ("Availability of the primary analytic indicators", [
        (
            "The four non-redundant analytic indicators were jointly available for 163 of 197 countries and territories (82.7%) (Figure 1A; Table 1). Current female smoking was available for 165/197, clean-fuel access for 191/197, ambient PM2.5 for 195/197, and age-standardised incidence of tracheal, bronchus, and lung cancer among women for 194/197 (Figure 1B). Among the 34 countries outside the complete frame, 28 were missing only the female smoking indicator, two were missing clean-fuel access only, one was missing both, and three had other multi-indicator patterns (Supplementary Table S4)."
        ),
        (
    ...
    ("Magnitude of measurable exposure inequalities", [
        ...
        (
            "Median ambient PM2.5 was 34.9 µg/m³ in low-income and 10.7 µg/m³ in high-income countries, a 24.1 µg/m³ difference (95% CI, 14.8 to 28.6; Cliff's delta, 0.79; 95% CI, 0.65 to 0.92). The distribution was less monotonic across intermediate income groups than the clean-fuel gradient (Figure 3B). Exposure and incidence distributions also varied by WHO region (Supplementary Figure S4; Supplementary Table S6)."
        ),
        (
            "Residence was the most consistently available within-country stratifier. The median urban-rural clean-fuel gap was 7.0 percentage points larger in low-income than high-income countries (95% CI, 2.2 to 18.1; Cliff's delta, 0.90; 95% CI, 0.78 to 1.00) (Figure 3C-D; Table 2; Supplementary Figure S5A). The smaller median gap in low-income than lower-middle-income settings did not imply greater equity: many low-income countries had low access in both urban and rural populations. Clean-fuel deficit correlated inversely with SDI (rho = -0.898, 95% CI, -0.925 to -0.859), HDI (rho = -0.909, 95% CI, -0.932 to -0.869), and urbanisation (rho = -0.663, 95% CI, -0.740 to -0.571), and positively with the rural clean-fuel gap (rho = 0.709, 95% CI, 0.592 to 0.802) (Supplementary Figure S5B)."
        ),
    ]),
    ("Age-standardised incidence validation analysis", [
        (
            "Historical smoking prevalence among women was positively associated with age-standardised incidence. Among 163 countries with complete model variables, the unadjusted standardised coefficient was 0.60. After adjustment for log GDP, urbanisation, and WHO region, the coefficient was 0.323 (95% CI, 0.163 to 0.483; P < 0.001) (Figure 4A-B; Table 2; Supplementary Table S8). The model explained 56.4% of the variance in the outcome (R² = 0.564; adjusted R² = 0.541). Adding current clean-fuel deficit and PM2.5 did not improve fit: adjusted R² was 0.536 and AIC increased from 345.3 to 346.7 (Figure 4C)."
        ),
        (
            "Associations were similar for female smoking in 2000, 2005, and 2010; the coefficient for current smoking in 2022 was smaller (0.177, 95% CI, 0.017 to 0.338) (Supplementary Figure S5C; Supplementary Table S9). The historical-smoking estimate remained positive after additional adjustment for the proportion of the population aged 65 years or older, exclusion of 11 countries above the Cook threshold, restriction to populations of at least one million, and Conley-type spatial covariance estimation (Supplementary Figure S5D; Supplementary Tables S9-S10). Model residuals showed modest spatial autocorrelation (Moran's I = 0.106; permutation P = 0.009), visible as a non-random geographic pattern in Figure 4D. Spatial sensitivity intervals were therefore reported, and model residuals were not assumed to be fully independent (Supplementary Figures S6 and S8)."
        ),
    ]),
...
DISCUSSION = [
    (
        "The usable data were concentrated at the exposure end of the pathway. Current female smoking, clean-fuel access, ambient PM2.5, and age-standardised incidence could be combined for 163 countries and territories, whereas comparable information was substantially more limited for second-hand and other long-latency exposures, screening, stage, treatment, survival, and socioeconomically stratified lung cancer outcomes. Such data often exist in surveys, registries, or programme systems, but inconsistent definitions, subgroup structures, and formats prevent their integration into a common inequality analysis."
    ),
    (
    ...
    (
        "The study has several limitations. Source identification was structured but did not systematically review every national registry or programme. Two investigators coded the audit independently with complete agreement, although their records were consolidated into the archive after coding rather than time-stamped prospectively during source identification. Several inputs were modelled estimates, and harmonised uncertainty draws were unavailable across data streams. Complete-case analysis underrepresented some lower-development settings. Model residuals showed spatial autocorrelation, although the smoking estimate remained positive with Conley-type covariance. The ecological design cannot resolve individual exposure, susceptibility, care access, or causal mechanisms."
    ),
    (
        "A minimum global indicator set could connect exposure surveillance with cancer care. Alongside age-standardised prevalence of active smoking, access to clean fuels and technologies for cooking, and ambient PM2.5 concentrations, such a set would require harmonised women-specific indicators of second-hand smoke exposure, occupational carcinogen exposure, and radon, together with indicators of LDCT eligibility and uptake, stage at diagnosis, histology, treatment, and survival stratified by wealth, education, and residence. Versioned metadata should identify the observation year, subgroup denominator, standard population, uncertainty, and whether each estimate is observed or modelled. HIDR and HEAT already support exposure-side comparisons; interoperable registry, screening, and survival systems are needed to extend them across the pathway."
    ),
]

CONCLUSION = (
    "Selected exposure and incidence indicators support broad country-level comparison of lung cancer prevention among women, but care pathway and socioeconomically stratified outcome data remain fragmented. Missingness varied with income and development, while smoking, clean-fuel access, and ambient PM2.5 followed distinct inequality gradients. Historical smoking prevalence among women was associated with age-standardised incidence, but this association and the environmental residual analyses are not causal estimates. Public-health action can address the exposure inequalities already measurable while data systems are strengthened for screening, diagnosis, treatment, and survival."
)
...
def main_table1(country: pd.DataFrame) -> list[list[str]]:
    return [
        ...
        ["Ambient PM2.5", "WHO GHO; 2019", "National annual mean modelled concentration (µg/m³)", "195/197", "Country; limited residence", "Primary exposure", "National modelled concentration; no personal or long-lag exposure"],
        ["Lung cancer incidence among women", "IHME/GBD-linked; 2021", "Age-standardised incidence per 100,000", "194/197", "Sex", "Primary burden anchor", "Modelled estimate; uncertainty draws not retained in the analytic file"],
...
FIGURE_LEGENDS = [
    "Figure 1. Availability and patterned missingness in the four-indicator analytic frame. (A) Country and territory missingness patterns for current female smoking, clean-fuel access, ambient PM2.5, and age-standardised lung cancer incidence among women. Grey map areas were not matched to the analytic frame and are not counted as missing. (B) Indicator-specific availability. The dashed line marks the 163-country complete overlap and is not an adequacy threshold. (C) Complete-frame inclusion by World Bank income group. (D) Median HDI and SDI differences between excluded and included countries; points are excluded-minus-included differences and lines are 95% CIs based on 5000 bootstrap replicates. LIC, low-income country; LMIC, lower-middle-income country; UMIC, upper-middle-income country; HIC, high-income country.",
    ...
    "Figure 4. Historical smoking prevalence among women and age-standardised lung cancer incidence. (A) Unadjusted country-level association between the 2000-2010 proxy for smoking among women and 2021 age-standardised incidence of tracheal, bronchus, and lung cancer among women; colours denote WHO regions. (B) Frisch-Waugh-Lovell partial-regression display after adjustment of both variables for log GDP, urbanisation, and WHO region. The line and shaded band show the adjusted fit and a pointwise 95% confidence band based on HC3 standard errors; the annotated coefficient is from the prespecified primary model. (C) R² and AIC for nested models. (D) Geographic distribution of primary-model residuals; positive values indicate incidence higher than predicted by the model and negative values indicate lower incidence. Colour limits are symmetric at the 95th percentile of absolute residuals, with more extreme values shown at the darkest endpoint colours. Residuals are model diagnostics, not estimates of omitted exposures or causal effects. Grey denotes no mapped model residual. All model variables are standardised.",
]
...
def supplementary_markdown(tables: list[tuple[str, list[list[str]], str]]) -> str:
    methods = [
        "### Scope and source identification\nThe supplement documents the structured source audit and the analyses supporting the main manuscript. Formal coding used WHO HIDR/HEAT-linked sources, GHO, GHE, GBD-linked outputs, World Bank indicators, and Global Data Lab HDI. Targeted verification covered IARC/GLOBOCAN, CanScreen5, CONCORD, WHO second-hand smoke metadata, and GTSS/GATS. Shen Wang and Jing Zhou independently coded all 180 component-by-dimension cells. Their records were archived after completion, compared without alteration, and showed 100% agreement (Cohen's κ = 1.00); no adjudication was required. Supplementary Figure S1 and Supplementary Tables S1-S3 give the workflow, decision rules, and item-level records.",
        "### Missingness and descriptive analyses\nThe primary frame required current female smoking, clean-fuel access, ambient PM2.5, and age-standardised lung cancer incidence among women. Missingness patterns were mutually exclusive. Coverage percentages used countries in each income or WHO-region stratum as denominators. Country-level values were not population weighted. Full country membership, coverage, and distributions appear in Supplementary Tables S4-S6 and Supplementary Figures S2-S4.",
        ...
        "### Age-standardised incidence models\nThe validation model used z-scored 2021 age-standardised incidence among women. Historical smoking was the mean female age-standardised current tobacco prevalence in 2000, 2005, and 2010. The primary model adjusted for z-scored log GDP, z-scored urbanisation, and WHO region, with HC3 standard errors. The adjusted relationship in Figure 4B was visualised using the Frisch-Waugh-Lovell construction: outcome and exposure were separately residualised on the adjustment set and the residuals were related using an HC3 fit. Alternative windows, adjustment for the proportion aged 65 years or older, Cook-distance exclusion, population restriction, and Conley-type spatial covariance are reported in Supplementary Figure S5C-D and Supplementary Tables S8-S10; general diagnostics and spatial residual checks appear in Supplementary Figures S6 and S8.",
    ...
    results = [
        "### Coverage and audit results\nMost excluded countries were missing female smoking, and complete-frame inclusion was lowest in low-income and income-unclassified settings (Supplementary Figure S2; Supplementary Tables S4-S5). The two independent coders agreed on all 180 cells (Cohen's κ = 1.00) (Supplementary Figure S1; Supplementary Table S3). Three exposure indicators met the analytic criteria; second-hand smoke, occupational carcinogens, and radon were limited. Screening, stage, and treatment sources were located but not sufficiently integrated, while survival and subtype data remained limited (Supplementary Figure S3; Supplementary Table S3).",
